...
GSYN_EXC = 'gsyn_exc'
GSYN_INH = 'gsyn_inh'
TAU_SYN_NE = 'tau_syn_Ne'
TAU_SYN_NI = 'tau_syn_Ni'
GSYN_N_EXC = 'gsyn_Ne'
GSYN_N_INH = 'gsyn_Ni'
...
class _EXP_TYPES(Enum):
    ...
    NE_DECAY = (7, DataType.UINT32)
    NE_INIT = (8, DataType.UINT32)
    NI_DECAY = (9, DataType.UINT32)
    NI_INIT = (10, DataType.UINT32)
    INITIAL_N_EXC = (11, DataType.S1615)
    INITIAL_N_INH = (12, DataType.S1615)
...
class SynapseTypeExponentialNMDA(AbstractSynapseType, AbstractContainsUnits):
    __slots__ = [
        "_data",
        "_n_neurons",
        "_units"]

    def __init__(self, n_neurons, tau_syn_E, tau_syn_I, tau_syn_nmda_E, tau_syn_nmda_I,
                 initial_input_exc=0.0, initial_input_inh=0.0,
                 initial_input_nmda_exc=0.0, initial_input_nmda_inh=0.0):
        ...
        self._data[TAU_SYN_NE] = tau_syn_nmda_E
        self._data[TAU_SYN_NI] = tau_syn_nmda_I
        self._data[GSYN_N_EXC] = initial_input_nmda_exc
        self._data[GSYN_N_INH] = initial_input_nmda_inh
    ...
    # ----------------------------------------------------------
    @property
    def tau_syn_nmda_E(self):
        return self._data[TAU_SYN_NE]

    @tau_syn_nmda_E.setter
    def tau_syn_nmda_E(self, new_value):
        self._data.set_value(key=TAU_SYN_NE, value=new_value)

    @property
    def tau_syn_nmda_I(self):
        return self._data[TAU_SYN_NI]

    @tau_syn_nmda_I.setter
    def tau_syn_nmda_I(self, new_value):
        self._data.set_value(key=TAU_SYN_NI, value=new_value)

    @property
    def isyn_nmda_exc(self):
        return self._data[GSYN_N_EXC]

    @isyn_nmda_exc.setter
    def isyn_nmda_exc(self, new_value):
        self._data.set_value(key=GSYN_N_EXC, value=new_value)

    @property
    def isyn_nmda_inh(self):
        return self._data[GSYN_N_INH]

    @isyn_nmda_inh.setter
    def isyn_nmda_inh(self, new_value):
        self._data.set_value(key=GSYN_N_INH, value=new_value)
    # ----------------------------------------------------------
    # ----------------------------------------------------------
    ...
    def get_synapse_targets(self):
        return "excitatory", "inhibitory", "NMDAexc", "NMDAinh"
    ...
    def get_synapse_type_parameters(self, machine_time_step):
        ...
        Ne_decay, Ne_init = get_exponential_decay_and_init(
            self._data[TAU_SYN_NE], machine_time_step)
        Ni_decay, Ni_init = get_exponential_decay_and_init(
            self._data[TAU_SYN_NE], machine_time_step)
        return [
            ...
            # ---------------------------------------------------------
            NeuronParameter(Ne_decay, _EXP_TYPES.NE_DECAY.data_type),
            NeuronParameter(Ne_init, _EXP_TYPES.NE_INIT.data_type),
            NeuronParameter(Ni_decay, _EXP_TYPES.NI_DECAY.data_type),
            NeuronParameter(Ni_init, _EXP_TYPES.NI_INIT.data_type),
            NeuronParameter(
                self._data[GSYN_N_EXC], _EXP_TYPES.INITIAL_N_EXC.data_type),
            NeuronParameter(
                self._data[GSYN_N_INH], _EXP_TYPES.INITIAL_N_INH.data_type)
        ]
