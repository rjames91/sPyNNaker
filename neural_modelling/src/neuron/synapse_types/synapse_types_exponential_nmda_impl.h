...
#ifndef _SYNAPSE_TYPES_EXPONENTIAL_NMDA_IMPL_H_
#define _SYNAPSE_TYPES_EXPONENTIAL_NMDA_IMPL_H_
...
typedef struct synapse_param_t {
    ...
    decay_t nmda_exc_decay;
    decay_t nmda_exc_init;
    decay_t nmda_inh_decay;
    decay_t nmda_inh_init;
    input_t input_buffer_nmda_exc_value;
    input_t input_buffer_nmda_inh_value;
} synapse_param_t;

typedef enum input_buffer_regions {
    EXCITATORY, INHIBITORY, NMDAe, NMDAi
...
static inline void synapse_types_shape_input(
    ...
    parameter->input_buffer_nmda_exc_value = decay_s1615(
        parameter->input_buffer_nmda_exc_value,
        parameter->nmda_exc_decay);
        
    parameter->input_buffer_nmda_inh_value = decay_s1615(
        parameter->input_buffer_nmda_inh_value,
        parameter->nmda_inh_decay);
}
...
static inline void synapse_types_add_neuron_input(
    ...
    else if (synapse_type_index == NMDAe) {
        parameter->input_buffer_nmda_exc_value = \
            parameter->input_buffer_nmda_exc_value + \
            decay_s1615(input, parameter->nmda_exc_init);

    } else if (synapse_type_index == NMDAi) {
        parameter->input_buffer_nmda_inh_value = \
            parameter->input_buffer_nmda_inh_value + \
            decay_s1615(input, parameter->nmda_inh_init);
    }
...
static inline void synapse_types_print_input(
        synapse_param_pointer_t parameter) {
    io_printf(
        IO_BUF, "E%12.6k - I%12.6k - Ne%12.6k - Ni%12.6k",
        parameter->input_buffer_excitatory_value,
        parameter->input_buffer_inhibitory_value,
        parameter->input_buffer_nmda_exc_value,
        parameter->input_buffer_nmda_inh_value
...
#endif  // _SYNAPSE_TYPES_EXPONENTIAL_NMDA_IMPL_H_
